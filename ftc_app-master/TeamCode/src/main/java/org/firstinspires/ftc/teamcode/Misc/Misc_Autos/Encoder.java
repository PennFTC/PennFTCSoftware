...
public class Encoder extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        if (colorS.blue() > colorS.red()) {
            encoderDrive(driveSpeed,  -1,  -1, 0.5);  // backward march
            stap();
            thwack.setPosition(0);
            sleep(500);  // if the color sensor on the arm sees the blue jewel then it moves backwards knocking the red off
            encoderDrive(driveSpeed, 4, 4, 2.0);
            stap();
        } else {
            encoderDrive(driveSpeed,  1,  1, 0.5); // forward march
            stap();
            thwack.setPosition(0);
            sleep(500);
        }
